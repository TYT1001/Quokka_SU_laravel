    ...
    <div class="container">
        ...
        <div class="d-flex justify-content-between container py-3">
            ...
            <form action="" class="flex justify-center">
                <input type="text" class="input input-bordered me-3 bg-indigo-300" name="key" placeholder="Search" value="{{request('key')}}">
        ...
        <div class="my-3 conatiner flex">
            ...
            <div class="form-control  max-w-xs mt-3 col-4">
                <form action="{{route('admin#nonCdmUpdate')}}" method="post">
                    @csrf
                    <input type="hidden" name="updated_id" value="{{$updatedData->id}}">
                    <div>
                        <label class="label">
                            <span class="label-text">Name</span>
                        </label>
                        <input name="name" type="text" value="{{$updatedData->name}}" placeholder="Type here" class="input w-full max-w-xs bg-indigo-300  placeholder:text-slate-50" />
                        @error('name')
                    ...
                    <div>
                        ...
                        <select name="year" class="select select-bordered w-full bg-indigo-300">
                            ...
                            @foreach ($year as $y)
                                <option value="{{$y}}" @if($y== $updatedData->year) selected @endif>{{$y}}</option>
                            @endforeach
                        </select>
                        @error('year')
                            <small class="text-danger">{{$message}}</small>
                    ...
                    <div>
                        ...
                        <select name="uni_id" class="select select-bordered w-full bg-indigo-300">
                            @foreach ($unis as $d)
                            <option value="{{$d->id}}" @if ($d->id == $updatedData->uni_id)
                                selected
                            @endif>{{$d->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <button class="btn btn-teal-100 my-3 btn-sm" type="submit">Update</button>
                </form>
            </div>
        </div>
